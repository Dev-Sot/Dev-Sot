<div align="center">

<img src="https://github.com/Dev-Sot/PortafolioNG/blob/main/public/images/portafolioimagen.gif?raw=true" width="100%" alt="Dev-Sot — portfolio preview" />

<h1>Dev-Sot</h1>
<p>Full Stack Software Engineer — backend architecture, SaaS platforms, web and mobile applications.</p>

<a href="https://portafolio-ng-gamma.vercel.app/">Portfolio</a> ·
<a href="mailto:sotelo.dev1@gmail.com">Email</a>

</div>

<br>

I build systems end to end: data modeling, API design, frontend, containerization and deployment. My focus is on code that stays maintainable after the first version ships — clear architecture, sensible defaults, and fewer moving parts rather than more.

Outside of client work I run **Extracurricular**, an independent studio publishing consumer apps on Google Play.

---

## Selected work

<table>
<tr>
<td width="50%">
<img src="https://portafolio-ng-gamma.vercel.app/images/projects/differential.png" width="100%" alt="Differential" />
</td>
<td valign="top" width="50%">

### Differential
Asistente médico de diagnóstico diferencial con RAG — evalúa síntomas contra 14 libros de medicina indexados (61.440 fragmentos), combinando búsqueda vectorial FAISS y BM25 con re-ranking, y entrega un diagnóstico fundamentado vía LLM.

`Python` · `Flask` · `FAISS` · `TypeScript`

[Ver proyecto →](https://portafolio-ng-gamma.vercel.app/projects)

</td>
</tr>
<tr>
<td width="50%">
<img src="https://github.com/Dev-Sot/zombie-night/raw/main/docs/social-preview.png" width="100%" alt="Noche Sin Luna" />
</td>
<td valign="top" width="50%">

### Noche Sin Luna
Supervivencia zombie top-down en pixel art, para jugar solo o cooperativo online de hasta 4 jugadores por WebRTC. Nueve noches en tres mundos con historia y objetivos propios, motor con bucle de paso fijo a 60 Hz y pathfinding por campo de flujo. Sin build ni dependencias — módulos ES nativos.

`JavaScript` · `HTML5 Canvas` · `WebRTC` · `Web Audio API`

[Jugar ahora →](https://zombie-night.vercel.app) · [Repositorio →](https://github.com/Dev-Sot/zombie-night)

</td>
</tr>
<tr>
<td width="50%">
<img src="https://github.com/Dev-Sot/PortafolioNG/blob/main/public/images/portafolioimagen.gif?raw=true" width="100%" alt="Dev-Sot Portfolio" />
</td>
<td valign="top" width="50%">

### Portfolio
Personal site and project archive, dark-mode-first. Built around typography, motion and information hierarchy instead of a template — case studies for every project above, an experience section and a working contact flow.

`Next.js` · `React` · `Tailwind CSS`

[About me →](https://portafolio-ng-gamma.vercel.app/#about)

</td>
</tr>
</table>

**Más proyectos:** [Hotel Booking](https://github.com/Dev-Sot/hotel-booking) (Next.js · Supabase · PostgreSQL · [demo](https://hotel-booking-gamma-red.vercel.app)) · [Canchazo](https://github.com/Dev-Sot/canchazo) (React · Supabase · Wompi) · [Aurum](https://github.com/Dev-Sot/Aurum-Restaurant) (React · Framer Motion) · [Pixel Dash](https://github.com/Dev-Sot/pixel-dash) (JavaScript · Canvas)

[→ Ver el portafolio completo](https://portafolio-ng-gamma.vercel.app/projects)

---

## Stack

**Languages:** Java, Kotlin, Dart, TypeScript, JavaScript, Python

**Frontend / Mobile:** React, Next.js, Tailwind CSS, Flutter, Android (Kotlin)

**Backend:** Spring Boot, Node.js, FastAPI

**Data:** PostgreSQL, MySQL, Redis, Firebase

**Infrastructure:** Docker, Linux, Git, GitHub Actions

---

## Apps — Extracurricular

Independent studio, consumer Android apps published on Google Play.

- **Bloom** — hábitos, estado de ánimo, diario y objetivos. Próximamente en Google Play. `Flutter`, `Dart`
- **Noir Wallpapers** — wallpapers curados con wallpaper del día y rachas. Próximamente en Google Play. `Flutter`, `Dart`

---

## Contact

[sotelo.dev1@gmail.com](mailto:sotelo.dev1@gmail.com) · [Portfolio](https://portafolio-ng-gamma.vercel.app/) · [GitHub](https://github.com/Dev-Sot?tab=repositories)
